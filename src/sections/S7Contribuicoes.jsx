...
export default function S7Contribuicoes() {
  return (
    <>
          ...
          {c.numeros.map((x, i) => (
            <div
              ...
              style={{
                ...
                color: ehEscura(x.cor) ? "#fff" : "var(--color-terra-900)",
              }}
            >
      ...
      <Reveal>
        <h2 className="mt-10 mb-1 font-display text-2xl font-bold text-terra-800">{c.eixos.titulo}</h2>
        <p className="mb-4 text-sm text-terra-700/80">{c.eixos.descricao}</p>
      </Reveal>
        ...
        {c.eixos.itens.map((x, i) => (
          <Reveal key={x.eixo} delay={i * 80}>
            <Sanfona
              ...
              cor={x.destaque ? cor("brasa") : cor("areia")}
      ...
      <div className="mt-6 space-y-3">
          ...
          >
            <div className="grid gap-3 p-5 md:grid-cols-2">
              {c.limitacoes.itens.map((l) => (
                <div key={l} className="rounded-lg border-l-4 border-rubro-600 bg-areia-100 p-4">
                  <p className="text-sm leading-relaxed text-terra-800">{l}</p>
                </div>
              ))}
            </div>
          </Sanfona>
        ...
        <Reveal delay={80}>
          ...
          >
            <div className="p-6">
              <ol className="relative space-y-4 border-l-2 border-ambar-400 pl-6">
                {c.agenda.itens.map((a, i) => (
                  <li key={a.titulo} className="relative">
                    <span className="absolute top-1.5 -left-[1.93rem] flex h-5 w-5 items-center justify-center rounded-full bg-brasa-500 font-display text-[11px] font-bold text-white">
                      {i + 1}
                    </span>
                    <h3 className="font-display text-base font-bold text-terra-900">{a.titulo}</h3>
                    <p className="mt-0.5 text-sm leading-relaxed text-terra-800/85">{a.texto}</p>
                  </li>
                ))}
              </ol>
            </div>
      ...
      <Reveal>
        <Cartao tom="escuro" className="mt-10 p-8 text-center">
          <p className="text-xs font-semibold tracking-[0.2em] text-ambar-300 uppercase">{c.fecho.rotulo}</p>
