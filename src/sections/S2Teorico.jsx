          ...
          return (
            <button
              ...
              className={`cursor-pointer rounded-full border px-4 py-1.5 text-sm font-semibold transition-all ${
                ativo ? "border-transparent text-white shadow-sm" : "border-areia-200 bg-white text-terra-800 hover:border-ambar-400"
              }`}
              style={ativo ? { backgroundColor: COR_GRUPO[g] ?? cor("terraEscuro") } : undefined}
            >
              {g}
            </button>
          );
        ...
        {lista.map((x, i) => (
          <Reveal key={x.termo} delay={i * 50}>
            <Cartao className="group h-full overflow-hidden p-0 transition-shadow hover:shadow-md">
              ...
              <div className="p-5">
                <h3 className="font-display text-lg leading-snug font-bold text-terra-900">{x.termo}</h3>
                <p className="mt-2 text-sm leading-relaxed text-terra-800/85">{x.definicao}</p>
                <p
                  className="mt-4 border-t border-areia-200 pt-3 text-xs font-semibold tracking-wide"
