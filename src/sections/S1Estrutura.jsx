...
export default function S1Estrutura() {
  ...
  return (
    <>
      ...
      {/* Cascata dedutiva */}
      <Reveal>
        <h2 className="font-display text-2xl font-bold text-terra-800">{c.cascata.titulo}</h2>
        <p className="mt-1 mb-5 max-w-2xl text-sm text-terra-700/80">{c.cascata.descricao}</p>
      </Reveal>
            ...
            <Reveal key={etapa.nivel} delay={i * 90}>
              ...
              >
                <div
                  ...
                  style={{
                    borderLeftColor: cor(c.cascata.cores[i] ?? "terra"),
                    backgroundColor: ativo ? "#fff" : "var(--color-areia-100)",
                  }}
                >
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <h3 className="font-display text-lg font-bold text-terra-900">{etapa.nivel}</h3>
                    <Etiqueta cor="var(--color-areia-200)">{etapa.escopo}</Etiqueta>
                  </div>
                  <div className="grid transition-all duration-400" style={{ gridTemplateRows: ativo ? "1fr" : "0fr" }}>
                    <div className="overflow-hidden">
                      <p className="pt-2 text-sm leading-relaxed text-terra-800/90">{etapa.texto}</p>
                    </div>
                  </div>
                  {!ativo && <p className="truncate pt-1 text-sm text-terra-700/50">{etapa.texto}</p>}
                </div>
              </button>
            </Reveal>
          );
      ...
      <Reveal>
        <Cartao tom="escuro" className="mt-12 p-6">
          <p className="text-xs font-semibold tracking-[0.18em] text-ambar-300 uppercase">
      ...
      <Reveal>
        <h2 className="mt-12 font-display text-2xl font-bold text-terra-800">
          {c.objetivosEspecificos.titulo}
        </h2>
        <p className="mt-1 mb-4 text-sm text-terra-700/80">{c.objetivosEspecificos.descricao}</p>
      </Reveal>
      <div className="space-y-2">
        {c.objetivosEspecificos.itens.map((o, i) => (
          <Reveal key={o.id} delay={i * 55}>
            <div className="flex items-start gap-4 rounded-lg border border-areia-200 bg-white p-4">
              <span
              ...
              <div className="min-w-0">
                ...
                <p className="mt-1 text-sm leading-relaxed text-terra-800">{o.texto}</p>
              </div>
            </div>
          </Reveal>
        ))}
